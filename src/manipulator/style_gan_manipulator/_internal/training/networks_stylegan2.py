...
try:
    ...
    from src.manipulator.style_gan_manipulator._internal.torch_utils import misc, persistence
    from src.manipulator.style_gan_manipulator._internal.torch_utils.ops import (
        conv2d_resample,
        upfirdn2d,
        bias_act,
        fma,
...
class FullyConnectedLayer(torch.nn.Module):
    def __init__(
        ...
        self.bias = (
            torch.nn.Parameter(torch.full([out_features], np.float32(bias_init))) if bias else None
        )
...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode="random", fused_modconv=True, gain=1):
        ...
        if self.use_noise and noise_mode == "random":
            noise = (
                torch.randn([x.shape[0], 1, self.resolution, self.resolution], device=x.device)
                * self.noise_strength
            )
...
class ToRGBLayer(torch.nn.Module):
    ...
    def forward(self, x, w, fused_modconv=True):
        ...
        x = modulated_conv2d(
            x=x, weight=self.weight, styles=styles, demodulate=False, fused_modconv=fused_modconv
        )
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(
        ...
        if self.in_channels == 0:
            ...
            x = self.conv1(
                x, next(w_iter), fused_modconv=fused_modconv, gain=np.sqrt(0.5), **layer_kwargs
            )
            x = y.add_(x)
        else:
            x = self.conv0(x, next(w_iter), fused_modconv=fused_modconv, **layer_kwargs)
...
class Discriminator(torch.nn.Module):
    def __init__(
        ...
        channels_dict = {
            res: min(channel_base // res, channel_max) for res in self.block_resolutions + [4]
        }
        fp16_resolution = max(2 ** (self.img_resolution_log2 + 1 - num_fp16_res), 8)

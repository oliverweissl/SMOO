...
# Produces experiments/auxiliary_files/homophone_mapping.json and synonym_mapping.json.
set -euo pipefail
cd "$(dirname "$0")"
...
PYTHONPATH=python -m initialize.build_label_mapping
PYTHONPATH=python -m initialize.generate_mappings "$@"

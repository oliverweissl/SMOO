...
from defaults.mmm._experiment_config import ExperimentConfig
from defaults.mmm._mmm_tester import MMMTester
...
def _build_manipulator() -> MultimodalManipulator:
    return MultimodalManipulator(
...
def _infer_dims(manipulator: MultimodalManipulator) -> tuple[int, int]:
    ...
    if image_dim <= 0 or text_dim <= 0:
        raise ValueError(f'Failed to infer MMM manipulator dimensions: image_dim={image_dim}, text_dim={text_dim}')
...
def _build_sut(args: argparse.Namespace) -> VLMSUT:
    spec = copy.deepcopy(MODEL_SPECS[args.vlm])
    max_model_len = args.max_model_len if args.max_model_len is not None else spec.pop('max_model_len', None)
    return VLMSUT(
        model=spec['model'],
...
def _build_optimizer(pop_size: int, num_objectives: int, solution_shape: tuple[int, ...]) -> PymooOptimizer:
    params = copy.deepcopy(PYMOO_NSGA2_DEFAULT_PARAMS)
    params['algo_params']['pop_size'] = pop_size
...
def _build_objectives() -> CriterionCollection:
    return CriterionCollection(*MMM_OBJECTIVES)
...
def main() -> None:
    ...
    manipulator = _build_manipulator()
    ...
    optimizer = _build_optimizer(args.pop_size, objectives.num_objectives, solution_shape)
    early_termination = get_early_termination(
        VLMBBoxIoU(),
        lambda values: values <= args.early_stop_iou_max,
        fulfill='any',
    )

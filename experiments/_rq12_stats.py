...
Usage:  python experiments/_rq12_stats.py [--out DIR]
"""
...
from experiments import _analysis
from experiments._analysis import compute_swtd, compute_text_metrics
...
def load() -> pd.DataFrame:
    ...
    df = _analysis.compute_image_metrics(df)
    df = compute_text_metrics(df)
    df.loc[df["genome_mode"] == "image", "ned"] = np.nan
    df = compute_swtd(df)
    df = _analysis.add_cumulative_asr_outcomes(df, iou_threshold=0.25)
...
def main():
    ...
    ap.add_argument("--out", default=str(Path(__file__).parent / "stats"))
    args = ap.parse_args()
    out = Path(args.out)
